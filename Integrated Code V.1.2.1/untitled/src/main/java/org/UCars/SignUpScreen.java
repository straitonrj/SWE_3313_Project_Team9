...
public class SignUpScreen extends JFrame{
    ...
    public SignUpScreen(){
        ...
        createYourAccountButton.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                int Temp = Main.SignUp(UsernameSignUp.getText(),PasswordSignUp.getText());
                if(Temp == 1){
                    SignUpErrorMessage.setText("Username Taken");
                    SignUpErrorMessage.setVisible(true);
                } else if (Temp == 2) {
                    SignUpErrorMessage.setText("Password must contain a minimum of six characters.");
                    SignUpErrorMessage.setVisible(true);
                } else if (Temp==0) {
                    SignUpErrorMessage.setVisible(false);
                    Main.Login(UsernameSignUp.getText(),PasswordSignUp.getText());
